...
# Отчёты наблюдателя приезжают с VDS через Яндекс.Диск: папка терминала
# MQL5\Files синхронизируется целиком, копировальщик не нужен (w2w_sync.bat
# и w2w_sync_loop.bat удалены 13.09.2026). Если там пусто —
...
def data_age(st):
    """Возраст сводки в часах и можно ли ему верить.

    Наблюдатель 1.01+ пишет time_utc — момент, когда снимок сделан. Раньше
    было только time (время последней котировки), а оно замирает на выходных:
    по нему живой терминал неотличим от упавшего, поэтому со старой версией
    возраст не измеряем и тревогу о молчании не поднимаем вовсе."""
    utc = st.get('time_utc')
    if not utc:
        return 0.0, False
    try:
        t = datetime.strptime(utc, '%Y.%m.%d %H:%M:%S')
    except ValueError:
        return 0.0, False
